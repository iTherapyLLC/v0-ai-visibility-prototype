...
export default function GeoAuditPage() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      router.push("/login?redirect=/geo-audit")
      return
    }
  ...
  const handleAuditComplete = (id: string, url: string) => {
    console.log("[GeoAudit] handleAuditComplete called with id:", id, "url:", url)

    setAuditId(id)
    setWebsiteUrl(url)
    setView("loading")

    console.log("[GeoAudit] State updated - auditId:", id, "websiteUrl:", url)

    // Show loading animation for 3 seconds
    setTimeout(() => {
      console.log("[GeoAudit] Switching to dashboard view")
      setView("dashboard")
    }, 3000)
  }

  const handleBack = () => {
    console.log("[GeoAudit] Returning to landing view")
    setView("landing")
    setWebsiteUrl("")
    setAuditId("")
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FAF8F5]">
        <div className="text-center space-y-4">
          <div className="w-12 h-12 border-4 border-[#B87333] border-t-transparent rounded-full animate-spin mx-auto" />
          <p className="text-[#2C2C2C]/60">Verifying access...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      {view === "dashboard" && (
        <>
          {console.log("[GeoAudit] Rendering dashboard with auditId:", auditId, "websiteUrl:", websiteUrl)}
